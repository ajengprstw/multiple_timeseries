...
Most of the cases encountered were using univariate time series. Unfortunately, real world use cases don't work that way. When there are multiple variables at play, it would be inefficient to perform univariate time series, so we have to handle all of them at the same time using Multiple Time Series.
In this case, we are provided a walmart store sales dataset from  Kaggle (https://www.kaggle.com/c/walmart-recruiting-store-sales-forecasting/data). With this dataset, we are going to help them in solving store sales forecasting problems in order to improve their business strategies. We will solve this problem using direct forecasting method combined with machine learning method.

## Multiple Time Series
Multiple (Multivariate) Time Series has more than one time-dependent variable. Each variable depends not only on its past values but also has some dependency on other variables. This dependency is used for forecasting future values[1].
.........

## The purpose of `forecastML`
In this article, we are using package `forecastML` to perform Time Series Forecasting with Machine Learning methods. 
...
-to understand the steps of performing multivariate (multiple) time series forecasting
...
The number observation of `data_clean` and `data walmart the have same number. So we can say that we don't have missing dates.

```{r}
data_train <- forecastML::create_lagged_df(data_walmart, #ganti data train 
                                           type = "train", 
                                           outcome_col = 3,
                                           horizons = c(1,4,12),
                                           lookback = 1:48,
...
```{r}
data_pred_cv <- predict(model_results_cv, prediction_function = list(prediction_function), data = data_train) # data test
```

```{r}
dat_new <- data_pred_cv %>% 
...
```{r}
plot(data_pred_cv, facet = group ~ horizon, windows = 2)
```
...
```{r}
data_forecast <- forecastML::create_lagged_df(data_walmart, 
                                              type = "forecast", 
                                              outcome_col = 3,
                                              horizons = c(1,4,12), 
                                              lookback = 1:48,
...
```{r}
data_forecasts <- predict(model_results_cv, 
...
data_actual <- data_walmart %>% 
               filter(Date >= "2012-11-02")
actual_indices <- data_walmart %>% 
                 filter(Date >= "2012-11-02") %>% 
                 select(Date)

# Plot all final forecasts plus historical data.
plot(data_combined, data_actual = data_actual, actual_indices = actual_indices)
# masih error karena seharusnya yg di forecast itu data test nya, tanggal nya belum sesuai

```

```{r}
range(data_walmart$Date)
range(data_combined$forecast_period)
```
