...
In this case, we are provided a walmart store sales dataset from  Kaggle (https://www.kaggle.com/c/walmart-recruiting-store-sales-forecasting/data). With this dataset, we are going to help them in solving store sales forecasting problems in order to improve their business strategies.

## Multiple Time Series

## The purpose of `forecastML`
In this article, we are using package `forecastML` to perform Time Series Forecasting with Machine Learning methods. The purpose of `forecastML` is to simplify the process of multi-step-ahead forecasting with standard machine learning algorithms. `forecastML` supports lagged, dynamic, static, and grouping features for modeling single and grouped numeric or factor/sequence time series. In addition, simple wrapper functions are used to support model-building with most R packages. This approach to forecasting is inspired by Bergmeir, Hyndman, and Koo's (2018) paper entitled "A note on the validity of cross-validation for evaluating autoregressive time series prediction"[1](https://cran.r-project.org/web/packages/forecastML/index.html).

## Learning Objectives

The goal of this article is to help you:
-to understand the main idea of package `forecastML`
-to understand the workflow of multivariate (multiple) time series forecasting
-to ...
...
# Reference
- https://cran.r-project.org/web/packages/forecastML/index.html
