...
Most of the cases encountered were using univariate time series which only consists of 2 variables and 1 object. Unfortunately, real world use cases not always as simple as that. When there are multiple variables and objects at play, it would be inefficient to perform univariate time series, so we have to handle all of them at the same time using Multiple Time Series.
