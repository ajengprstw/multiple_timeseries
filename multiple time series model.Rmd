...
min_date <- min(data_clean$Date)
max_date <- max(data_clean$Date)
```
...
observasi data_clean dan data_walmart sama artinya tidak ada data yang hilang
```{r}
nrow(data_clean)
nrow(data_walmart)
```
...
```{r}
plot(data_pred_cv, facet = group ~ horizon, windows = 1)
```
